...
The peer distribution lives under [`peer/`](peer/README.md) in this repository. Peers can use its one-command bootstrap, paste an agent-assisted installation prompt, or follow the [manual setup guide](peer/SETUP.md). Installation detects Codex and Claude Code and registers the skill at user scope without enabling the dispatcher or creating project state. Tags matching `peer-vX.Y.Z` publish a peer-only GitHub Release archive and checksum.
