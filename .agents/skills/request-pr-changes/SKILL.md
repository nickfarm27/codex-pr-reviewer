...
The command verifies the reviewed head is still current and that GitHub still has the recorded pending review. It also reconciles any GitHub-side comment edits into local history before carrying the finding forward. Do not bypass a stale-head, missing-review, or unexpected-state failure. Never substitute `APPROVE` or a general comment review.
